...
//            std::std::cout << length_of_input_X << std::std::endl;
            input_diff[xor_X][col_index][0] = i;
            input_diff[xor_X][col_index][1] = j;
            input_diff[xor_X][col_index][2] = sBox[i];
            input_diff[xor_X][col_index][3] = sBox[j];
            input_diff[xor_X][col_index][4] = sBox[i] ^ sBox[j];

            std::cout<< xor_X<< " , " <<col_index << "\n";
        }
        col_index+= 1;
